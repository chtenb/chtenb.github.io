    ...
    .OnSuccess(email => SendEmail(email, server1)
      .OnFailure(ex => ex switch {
        ServerNotFoundException => 
          // We don't need to inspect the second exception,
...
But it demonstrates the concept of having subtracks very well.
...
Here is the code we've produced in this article series.
