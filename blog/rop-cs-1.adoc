...
public abstract class Result<TResult, TError> {
  ...
  public sealed class Failure : Result<TResult, TError> {
    ...
    public override Result<TNextResult, TError> OnSuccess<TNextResult>(
      Func<TResult, Result<TNextResult, TError>> onSuccess) => new Result<TNextResult, TError>.Failure(ErrorValue);
    public override TReturn Handle<TReturn>(
      Func<TResult, TReturn> onSuccess, Func<TError, TReturn> onFailure) => onFailure(ErrorValue);
  }
...
Result<Unit, Exception> SendEmail(EmailMessage email);
....
