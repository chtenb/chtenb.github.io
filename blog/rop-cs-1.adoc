...
In the link:/?page=rop-cs-2[next part] of this article series we will have a look at one way of implementing explicit failure in C#: a `Result` type with two subtypes, the `Success` type for the success path and the `Failure` type for the failure path.
...
In the link:/?page=rop-cs-2[next part] of this article series we will implement a `Result` type for C#.
